...
def quad_reg(x,y):
    ...
    x2 = x**2
    full_x = np.column_stack([x2,x])

    #Fit model.
    cmodel = LinearRegression().fit(full_x,y)

    #Extract coefficients and R2.
    b2 = cmodel.coef_[0]
    b1 = cmodel.coef_[1]
    b0 = cmodel.intercept_
    r2 = cmodel.score(full_x,y)
    return (b2,b1,b0,r2)

def one_perm(cobs,cperms):

    #One-sided test for one direction only.
...
def two_perm(cobs,cperms):
    ...
    #Two-sided test for two directions.
    nperm = cperms.shape[0]
    cp = (np.sum(np.abs(cperms)>=np.abs(cobs))+1)/(nperm+1)
    return(cp)


def quad_perm_sphere_p(x, y, perm_id, corr_type='quadreg', null_dist=False):
    """Generate a p-value for the spatial quadratic regression between two parcellated cortical surface maps (ADAPTED)
    ...
    perm_id : ndarray
        Array of permutations, shape = (m, nrot)
    corr_type : string, optional
        Correlation type {'quadreg'}. Default is 'quadreg'.
    null_dist : bool, optional
        Output null correlations. Default is False.
    ...
    -------
    b2_p : float
        Permutation p-value for B2
    b1_p : float
        Permutation p-value for B1
    b0_p : float
        Permutation p-value for B0
    r2_p : float
        Permutation p-value for R2
    ...
    * Váša F, Seidlitz J, Romero-Garcia R, Whitaker KJ, Rosenthal G, Vértes PE,
    ...
    # empirical quadratic regression
    b2,b1,b0,r2 = quad_reg(x,y)
    ...
    # quadratic regression with unpermuted measures
    null_xy = np.zeros((nperm,4))
    null_yx = np.zeros((nperm,4))
    for rr in range(nperm):
        null_xy[rr,:] = quad_reg(x_perm[:, rr],y)
        null_yx[rr,:] = quad_reg(x,y_perm[:, rr])

    # p-value definition depends on the empirical measure
    b2_p_xy = two_perm(b2,null_xy[:,0])
    b1_p_xy = two_perm(b1,null_xy[:,1])
    b0_p_xy = two_perm(b0,null_xy[:,2])
    r2_p_xy = one_perm(r2,null_xy[:,3])
    b2_p_yx = two_perm(b2,null_yx[:,0])
    b1_p_yx = two_perm(b1,null_yx[:,1])
    b0_p_yx = two_perm(b0,null_yx[:,2])
    r2_p_yx = one_perm(r2,null_yx[:,3])

    # average p-values
    b2_p = (b2_p_xy + b2_p_yx) / 2
    b1_p = (b1_p_xy + b1_p_yx) / 2
    b0_p = (b0_p_xy + b0_p_yx) / 2
    r2_p = (r2_p_xy + r2_p_yx) / 2

    return (b2_p,b1_p,b0_p,r2_p)
...
def quad_spin_test(map1, map2, surface_name='fsa5', parcellation_name='aparc', n_rot=1000,
              type='quadreg', null_dist=False, ventricles=False):
    """Spin permutation (ADAPTED)
    ...
    * Váša F, Seidlitz J, Romero-Garcia R, Whitaker KJ, Rosenthal G, Vértes PE,
    ...
    # p_spin, r_dist = perm_sphere_p(map1, map2, perm_id, type, null_dist=True)
    b2_spin,b1_spin,b0_spin,r2_spin = quad_perm_sphere_p(map1, map2, perm_id, type, null_dist=True)

    # if null_dist is True:
    #     return p_spin, r_dist
    # elif null_dist is not True:
    #     return p_spin
    return (b2_spin,b1_spin,b0_spin,r2_spin)
