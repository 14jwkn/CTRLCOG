...
# predicting each cognitive variable in the given batch using both average 
# and modal controllability calculated from the set of state matrices specified. 
# Reformat to generate percentages of the total accuracy score.
# Output:
...
if (statetype == 'SC_dFCcat') {
  feature_names <- c('ave_sc','mod_sc','ave_dFC','mod_dFC')
} else if (statetype == 'dFCcat') {
  feature_names <- c('ave_dFC','mod_dFC')
} else if (statetype == 'SC') {
  feature_names <- c('ave_sc','mod_sc')
} 
