...
This model predicts each cognitive variable in the given batch using controllability calculated with the continuous time system 
in the set of state matrices specified. CV is done for the given repetitions, and inner and outer
...
Usage: 
    LE_group_subcontrol_structfunc_KRR_score_cont.py <k> <sctype> <threstype> <thresval> <controltype> <statetype> <septype> <nrep> <inner_k> <outer_k>
...
if __name__ == '__main__':
    ...
    krrpath = (basepath+'/KRRXFS/cont_'+controltype+'_'+statetype+'_'+septype+'_'+
    ...
    for kidx in range(nk):
        ...
        predlabs = [(errstate+'_r'+str(ridx+1)) for ridx in range(nroi)]
    ...
    #If the error list contains SC. Generate SC predictor labels.
    errstate = 'SC'
    ...
    #If the error list contains sFC. Generate sFC predictor labels.
    errstate = 'sFC'
    ...
    #Read in the continuous values and divide into SC, sFC, and dFC.
    infile = (basepath+'/cont_'+controltype+'_all.csv')
    contmat = pd.read_csv(infile,index_col=0)
    contmat = contmat.loc[:,deslabs]
    sclabs = [x for x in deslabs if 'sc' in x]
    scmat = contmat.loc[:,sclabs]
    sFClabs = [x for x in deslabs if 'sFC' in x]
    sFCmat = contmat.loc[:,sFClabs]
    dFClabs = [x for x in deslabs if any(y in x for y in ['s'+str(i+1) for i in range(nk)])]
    dFCall = contmat.loc[:,dFClabs]

    #Extract subject list.
    sublist = contmat.index.tolist()
    ...
    #If doing a specific dFC state.
    else:

        #Extract state.
        predlabs = dFCall.columns.values.tolist()
        cpredlabs = [x for x in predlabs if ('s'+statetype) in x]
        predmat = dFCall.loc[:,cpredlabs]
        currsize = np.shape(predmat)[1]

        #Kernelize each slice.
        feature_names = [('s'+statetype)]
        n_features_list = [currsize]
        start_and_end = np.concatenate([[0],np.cumsum(n_features_list)])
        slices = [slice(start,end) 
                  for start,end in zip(start_and_end[:-1],start_and_end[1:])]
        nspace = len(feature_names)
