...
# of thresholding, the set of controllability, the set of matrices, the set of
# cognitive variables, CV repetitions, CV inner fold number, CV outer fold number,
# and permutation number, plot the regions with significant Haufe scores which are
# common and unique between average and modal controllability.
# Output:
# 'comsig_',base_outlab,cctrl,'_',ccog Regions which are significant for both.
# 'not_comsig_',base_outlab,cctrl,'_',ccog Regions which are not significant for both.
# 'comdir_',base_outlab,cctrl,'_',ccog Significant regions which do not have opposing signs between both.
# 'not_comdir_',base_outlab,cctrl,'_',ccog Significant regions which do have opposing signs between both.
# 'comsig_comdir_',base_outlab,cctrl,'_',ccog Regions which are significant and have the same sign for both.
# 'not_comsig_comdir_',base_outlab,cctrl,'_',ccog Regions which are not significant and have the same sign for both.
...
# #Create red-blue brain plotter with extreme values.
# rbXplot <- function(cstate,atlasord,base_atlas,plist,pltidx) {
#   
#   #Plot.
#   Score <- cstate
#   Score[Score > 0] <- 1
...
rblegend <- function(cstate,minscore,maxscore,atlasord,base_atlas) {
...
# #Create red-blue brain plotter.
# rbplot <- function(cstate,minscore,maxscore,atlasord,base_atlas,nacolor,rbcolors,plist,pltidx) {
# 
#   #Plot.
#   Score <- cstate
#   curr_v <- cbind(atlasord,Score)
...
#     scale_fill_gradientn(colors=rbcolors,
#                          limits=c(minscore,maxscore),
#                          breaks=c(minscore,0,maxscore),
#                          labels=format(c(minscore,0,maxscore)),
#                          na.value=nacolor) +
#     # scale_fill_scico(palette='berlin',
#     #                midpoint=0,
#     #                limits=c(minscore,mascore)) +
#     theme_classic() +
#     theme(plot.title = element_text(hjust=0.05),
#           axis.line.y = element_blank(),
#           axis.text.y = element_blank(),
...
# 
# #Create red-blue brain plotter with extreme values.
# rbXplot <- function(cstate,atlasord,base_atlas,nacolor,rbcolors,plist,pltidx) {
#   
#   #Plot.
#   Score <- cstate
#   Score[Score > 0] <- 1
...
# 
# #Generate red-blue legend.
# rblegend <- function(cstate,minscore,maxscore,atlasord,base_atlas,nacolor,rbcolors) {
#   
#   #Plot.
#   Score <- c(rep(minscore,180),rep(maxscore,180))
#   curr_v <- cbind(atlasord,Score)
...
max_plotter <- function(score_thres,minscore,maxscore,base_outfile,
  ...
  #Save.
  # do.call('grid.arrange',c(plist,ncol=9))
...
#Do common plots.
common_ctrl_plotter <- function(ccoglist,base_outlab,coutpath,
                           ccollector,cmaxmin,
                           ctrl_list,nfull,full_featlabs,
                           atlasord,base_atlas,nacolor,bcolor,xwidth,xheight,
                           states,nstates,nroi,
                           rbcolors,
                           netlabels,netlist,nnet,netmat,netsum,netcolors,
  ...
  ccols <- c()
  for (cctrl in ctrl_list) {for (ccog in ccoglist) {ccols <- c(ccols,paste0(cctrl,'.',ccog))}}
  nccols <- length(ccols)
  ...
  for (cctrl in ctrl_list) {
    for (ccog in ccoglist) {
      ccol_lab <- paste0(cctrl,'.',ccog)
      limvals <- c()
      for (cstate in states) {
        collect_lab <- paste0(cctrl,'.',cstate,'.',ccog)
  ...
  #Threshold for common significance.
  comp_rows <- rowSums(!is.na(threscollect))==ncol(threscollect) #Rows with all non-NA values, producing complete rows.
  comsig <- threscollect
  comsig[!comp_rows,] <- NA #Remove rows which are not complete.
  
  #Threshold for common direction, where one entry is not acceptable.
  posrows <- (threscollect>0)
  posrows[is.na(posrows)] <- F
  negrows <- (threscollect<0)
  negrows[is.na(negrows)] <- F
  noNArows <- rowSums(!is.na(threscollect))
  onedir_rows <- (rowSums(posrows)==noNArows)|(rowSums(negrows)==noNArows) #Row is all positive or all negative for non-NA values.
  oneonly_rows <- rowSums(!is.na(threscollect))==1 #Rows with only one non-NA value.
  comdir <- threscollect
  comdir[!onedir_rows,] <- NA #Remove rows which do not have common direction.
  comdir[oneonly_rows,] <- NA #Remove rows with a single entry.
 
  #Threshold for both common significance and common direction.
  comsig_comdir <- threscollect
  comsig_comdir[!comp_rows,] <- NA  #Remove rows which are not complete. Will already remove single entry rows.
  comsig_comdir[!onedir_rows,] <- NA #Remove rows which do not have common direction.

  #For each cognitive version, plot corresponding versions.
  for (ccog in ccoglist) {
    out_ctrl_list <- c('ave','mod')
    for (cctrl in out_ctrl_list) {
      ccol_lab <- paste0(cctrl,'.',ccog)
      print(ccol_lab)
      
      #Plot for common significance.
      score_thres <- comsig[,ccol_lab]
      if (cctrl=='mod') {
        score_thres <- -score_thres
      }
      base_outfile <- paste0(coutpath,'comsig_',base_outlab,cctrl,'_',ccog)
      minscore <- thresmaxmin['min',ccol_lab]
      maxscore <- thresmaxmin['max',ccol_lab]
      ...
      #Plot not common significance.
      notmat <- threscollect
      notmat[!is.na(comsig)] <- NA
      score_thres <- notmat[,ccol_lab]
      if (cctrl=='mod') {
        score_thres <- -score_thres
      }
      base_outfile <- paste0(coutpath,'not_comsig_',base_outlab,cctrl,'_',ccog)
      minscore <- thresmaxmin['min',ccol_lab]
      maxscore <- thresmaxmin['max',ccol_lab]
      max_plotter(score_thres,minscore,maxscore,base_outfile,
                  atlasord,base_atlas,nacolor,bcolor,xwidth,xheight,
      ...
      #Plot for common direction.
      score_thres <- comdir[,ccol_lab]
      if (cctrl=='mod') {
        score_thres <- -score_thres
      }
      base_outfile <- paste0(coutpath,'comdir_',base_outlab,cctrl,'_',ccog)
      minscore <- thresmaxmin['min',ccol_lab]
      maxscore <- thresmaxmin['max',ccol_lab]
      max_plotter(score_thres,minscore,maxscore,base_outfile,
                  atlasord,base_atlas,nacolor,bcolor,xwidth,xheight,
      ...
      #Plot not common direction.
      notmat <- threscollect
      notmat[!is.na(comdir)] <- NA
      score_thres <- notmat[,ccol_lab]
      if (cctrl=='mod') {
        score_thres <- -score_thres
      }
      base_outfile <- paste0(coutpath,'not_comdir_',base_outlab,cctrl,'_',ccog)
      minscore <- thresmaxmin['min',ccol_lab]
      maxscore <- thresmaxmin['max',ccol_lab]
      max_plotter(score_thres,minscore,maxscore,base_outfile,
                  atlasord,base_atlas,nacolor,bcolor,xwidth,xheight,
      ...
      #Plot for common significance and direction.
      score_thres <- comsig_comdir[,ccol_lab]
      if (cctrl=='mod') {
        score_thres <- -score_thres
      }
      base_outfile <- paste0(coutpath,'comsig_comdir_',base_outlab,cctrl,'_',ccog)
      minscore <- thresmaxmin['min',ccol_lab]
      maxscore <- thresmaxmin['max',ccol_lab]
      max_plotter(score_thres,minscore,maxscore,base_outfile,
                  atlasord,base_atlas,nacolor,bcolor,xwidth,xheight,
      ...
      #Plot not common significance and direction.
      notmat <- threscollect
      notmat[!is.na(comsig_comdir)] <- NA
      score_thres <- notmat[,ccol_lab]
      if (cctrl=='mod') {
        score_thres <- -score_thres
      }
      base_outfile <- paste0(coutpath,'not_comsig_comdir_',base_outlab,cctrl,'_',ccog)
      minscore <- thresmaxmin['min',ccol_lab]
      maxscore <- thresmaxmin['max',ccol_lab]
      max_plotter(score_thres,minscore,maxscore,base_outfile,
                  atlasord,base_atlas,nacolor,bcolor,xwidth,xheight,
...
outer_k <- args[10]
nperm <- args[11]
print(paste(k,sc_subgroup,sctype,threstype,thresval,controltype,statetype,septype,
            nrep,inner_k,outer_k,nperm,sep=' '))
...
full_featlabs <- paste0('sc_r',1:nroi)
...
outpath <- paste0(basepath,'KRRXFS/',controltype,'_',statetype,'_',septype,'_',
                  nrep,'_',inner_k,'_',outer_k,'_',sctype,'/plots/covha_BH_TwoP/')
...
#Produce column names for all versions of interest, including control, state, and cognition.
colcollect <- c()
for (cctrl in ctrl_list) {
  for (cstate in states) {
    for (ccog in coglist) {
      colcollect <- c(colcollect,paste0(cctrl,'.',cstate,'.',ccog))
    }
  }
}
...
#Generate path for these plots.
coutpath <- paste0(outpath,'common_ctrl/')
dir.create(coutpath)

#Find regions which are significant and have the same direction across AC-MC,
#and those which do not.
base_outlab <- ''
for (outcog in coglist) {
  ccoglist <- c(outcog)
  common_ctrl_plotter(ccoglist,base_outlab,coutpath,
                   ccollector,cmaxmin,
                   ctrl_list,nfull,full_featlabs,
                   atlasord,base_atlas,nacolor,bcolor,xwidth,xheight,
                   states,nstates,nroi,
