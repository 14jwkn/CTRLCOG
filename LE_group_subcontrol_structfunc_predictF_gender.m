%{
For the given gender, number of repetitions, outer loops, and inner loops for cross
...
participants.
Outputs:
gender,'_r',num2str(nrep),'_o',num2str(outer_k),'_i',num2str(inner_k),'_predict_splits.h5' Outer and inner cross validation subject indices.
gender,'_',num2str(nperm),'_perm_splits.h5' Permutation subject indices.
%}

%Define command line arguments.
function [] = LE_group_subcontrol_structfunc_predictF_gender(nrep,inner_k,outer_k,nperm,gender)
...
% gender = Gender.
disp(append('Doing: ',nrep,' ',inner_k,' ',outer_k,' ',nperm,' ',gender))
...
infile = append('dr_full_intersect_',gender,'.txt');
...
outfile = append(outpath,gender,'_r',num2str(nrep),'_o',num2str(outer_k),'_i',num2str(inner_k),'_predict_splits.h5');
...
outfile = append(outpath,gender,'_',num2str(nperm),'_perm_splits.h5');
