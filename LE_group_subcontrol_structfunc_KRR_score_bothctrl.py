...
fit a model with CV similar to the main analysis but with both average and modal 
controllability in the same model. Generate the CV accuracy for the test set, 
the CV accuracy for the inner CV for the best hyperparameter set, 
the decomposed CV accuracy for the test set for each feature band, the
decomposed CV accuracy after demeaning of the test set according to the 
decomposition assumptions, and the Haufe scores.

Usage: 
    LE_group_subcontrol_structfunc_KRR_score_bothctrl.py <k> <sctype> <threstype> <thresval> <controltype> <statetype> <septype> <nrep> <inner_k> <outer_k>
...
if __name__ == '__main__':
    ...
    #Read in the full group Gramian error table to find states with errors if not doing subject-wise.
    ...
    if controltype == 'avemod':
        ...
        dFCall = pd.concat((dFCave,dFCmod),axis=1)
    ...
    predlabs = []
    for cctrl in ['ave','mod']:
        for kidx in range(nk):
            for ridx in range(nroi):
                predlabs.append(cctrl+'_s'+str(kidx+1)+'_r'+str(ridx+1))
    ...
    for kidx in range(nk):
        ...
        if (errstate in gramstates):
            ...
            regrem = [('ave_'+errstate+'_r'+str(x)) for x in regrem] + [('mod_'+errstate+'_r'+str(x)) for x in regrem]
    ...
    if controltype == 'avemod':
        ...
        scmat = pd.concat((scave,scmod),axis=1)
        predlabs = ([('ave_sc_r'+str(ridx+1)) for ridx in range(nroi)] + 
                    [('mod_sc_r'+str(ridx+1)) for ridx in range(nroi)])
    ...
    if (errstate in gramstates):
        ...
        errmat = pd.read_csv(infile,index_col=None,header=0)
        regrem = errmat.loc[:,'Region'].values.tolist()
        regrem = [('ave_sc_r'+str(x)) for x in regrem] +  [('mod_sc_r'+str(x)) for x in regrem]
        newlabs = [x for x in predlabs if x not in regrem]
    ...
    if controltype == 'avemod':
        ...
        sFCmat = pd.concat((sFCave,sFCmod),axis=1)
        predlabs = ([('ave_sFC_r'+str(ridx+1)) for ridx in range(nroi)] +
                    [('mod_sFC_r'+str(ridx+1)) for ridx in range(nroi)])
    ...
    if (errstate in gramstates):
        ...
        errmat = pd.read_csv(infile,index_col=None,header=0)
        regrem = errmat.loc[:,'Region'].values.tolist()
        regrem = [('ave_sFC_r'+str(x)) for x in regrem] + [('mod_sFC_r'+str(x)) for x in regrem]
        newlabs = [x for x in predlabs if x not in regrem]
    ...
    dFC_size = int(np.shape(dFCall)[1]/2)
    sc_size = int(np.shape(scmat)[1]/2)
    sFC_size = int(np.shape(sFCmat)[1]/2)
    ...
    if statetype == 'SC_dFCcat':
        ...
        feature_names = ['ave_sc','mod_sc','ave_dFC','mod_dFC']
        n_features_list = [sc_size,sc_size,dFC_size,dFC_size]
    ...
    #If doing dFC.
        ...
        #Kernelize each slice.
        feature_names = ['ave_dFC','mod_dFC']
        n_features_list = [dFC_size,dFC_size]
        start_and_end = np.concatenate([[0],np.cumsum(n_features_list)])
        slices = [slice(start,end) 
                  for start,end in zip(start_and_end[:-1],start_and_end[1:])]
        nspace = len(feature_names)
    ...
    #If doing SC.
        ...
        #Kernelize each slice.
        feature_names = ['ave_sc','mod_sc']
        n_features_list = [sc_size,sc_size]
        start_and_end = np.concatenate([[0],np.cumsum(n_features_list)])
        slices = [slice(start,end) 
                  for start,end in zip(start_and_end[:-1],start_and_end[1:])]
        nspace = len(feature_names)
    ...
    rep_testacc_pr,rep_testacc,rep_inneracc,rep_testacc_derived,
    rep_splitacc,rep_splitacc_strict,rep_splitacc_svd,rep_splitacc_svd_strict, 
    rep_ha_feat,rep_covha_feat,rep_ynormha_feat,rep_covha_feat_derived] = overall_crossval(data_X,data_Y,data_C, #Data
                                                                    nrep,outer_k,inner_k,outercv_test,innercv_test, #CV items
                                                                    alpha_list,mykernel,feature_names,slices,nspace,ynorm, #Multi-KRR items
                                                                    repseed, #Random items
                                                                    xcon,ycon) #Preprocessing items
